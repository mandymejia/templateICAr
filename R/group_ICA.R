...
groupICA.cifti <- function(
  ...
  if(verbose){ cat(paste0("Number of subjects: ",N,"\n")) }

  if (N*num_PCs > max_rows_GPCA) {
    warning(
  ...
  for (ii in seq(N)) {
    ...
    #read in BOLD
    if (verbose) cat(paste0("Reading data for subject ",ii," of ",N, "\n"))
    fname_ii <- cifti_fnames[ii]

    # Smooth BOLD data. 
    # The call returns the path to the smoothed cifti, which will be in a temp dir.
    if (smooth) {
      fname_ii <- smooth_cifti(
        x = fname_ii, 
        cifti_target_fname=file.path(tempdir(), basename(fname_ii)),
    ...
    BOLD_ii <- as.matrix(read_cifti(fname_ii, brainstructures=brainstructures))
    ntime <- ncol(BOLD_ii)
    if (ii==1) {
      nvox <- nrow(BOLD_ii)
      ...
      if (nrow(BOLD_ii) != nvox) {
        stop('Number of brain locations in subject ',ii,' does not match that of first subject.')
      }
    }
    ...
    YtY <- crossprod(Y_ctr) #1 min
    Y_svd <- svd(YtY, nv = 0, nu=num_PCs_ii) #5 sec
    U_ii <- Y_svd$u
  ...
  # Adapts changes made for multiple runs
  ...
  if ("left" %in% brainstructures) xifti_GICA$data$cortex_left <- icasig[1:nleft,]  #[flat_bs_mask == "left",, drop=FALSE]
  if ("right" %in% brainstructures) xifti_GICA$data$cortex_right <- icasig[nleft+(1:nright),] #[flat_bs_mask == "right",, drop=FALSE]
  if ("subcortical" %in% brainstructures) xifti_GICA$data$subcort <- icasig[nleft+nright+(1:nsub),] #[flat_bs_mask == "subcortical",, drop=FALSE]
